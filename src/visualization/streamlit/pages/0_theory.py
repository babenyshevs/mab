...
st.markdown(
    """
    # Theory behind A/B Test and Reinforcement learning / Multi-Armed Bandit algorithms
    ---
    ## Summary

    1. **A/B Testing Purpose**: A/B testing compares different versions of a product or service to improve 
    business metrics like user engagement and satisfaction.

    2. **Reinforcement Learning Components**: RL involves an agent interacting with an environment to maximize 
    long-term objectives, with key components including the agent, environment, state, action, reward, and policy.

    3. **MAB Problem Definition**: The Multi-Armed Bandit problem involves an agent selecting actions with 
    unknown rewards to maximize total reward over time.

    4. **Exploration vs. Exploitation**: Balancing exploration (trying new options) and exploitation 
    (choosing known rewarding options) is crucial in both A/B testing and MAB problems.

    5. **Learning Methods**: Various learning methods, such as epsilon-greedy, UCB, and Thompson Sampling, 
    are used in both RL and MAB to optimize decision-making and maximize rewards.
